...
from dtPyLibs import utils
from dtPyLibs.utils import internet
...
class DownloadProgress:
    ...
    def log_progress(self, force=False):
        ...
        if force or (progress_details['percentage_complete'] % self.__log_every == 0 and 0 < progress_details['percentage_complete'] != self.__prior_perc and progress_details[
            ...
            logging.info(
                ...
                f'Average Download speed: {progress_details["download_rate_friendly"]} [Max: {utils.bytes_to_readable_unit(self.__max_download_rate)}, '
                f'Min: {utils.bytes_to_readable_unit(self.__min_download_rate)}]. '
    ...
    def progress(self):
        ...
        return {'config': self.config.as_json(), 'percentage_complete': perc_downloaded, 'download_rate_bytes': download_rate, 'time_remaining_seconds': time_remaining,
                'restarts_after_interruption': self.__restarts_after_interruption, 'download_rate_friendly': f'{utils.bytes_to_readable_unit(download_rate)} per second',
        ...
        try:
            ...
            while not self.complete:
                try:
                    ...
                    logging.info(f'Segment expected size: {utils.bytes_to_readable_unit(segment_size)}')
                    ...
                    if len(self.segment_range.split(':')) > 1:
                        ...
                        if os.path.exists(segment_partial_name):
                            ...
                            logging.info(f'Existing segment file size is: {utils.bytes_to_readable_unit(pathlib.Path(segment_partial_name).stat().st_size)}')
                        ...
                        logging.info(f'Requested start_range: {start_range}, original start_range: {self.segment_range.split(":")[1]}, end_range: {end_range}. '
                                     f'Estimated size of segment remaining to download: {utils.bytes_to_readable_unit(size_remaining)}')
                    ...
                    if pathlib.Path(segment_partial_name).stat().st_size < segment_size:
                        raise internet.IncompleteDownload(f'Destination file is {utils.bytes_to_readable_unit(pathlib.Path(segment_partial_name).stat().st_size)}, '
                                                          f'expected {utils.bytes_to_readable_unit(segment_size)}.')
                    elif 0 < segment_size < pathlib.Path(segment_partial_name).stat().st_size:
                        self.continue_retrying_on_interruption = False
                        message = f'Expected segment to be {utils.bytes_to_readable_unit(segment_size)}, but downloaded data is greater: ' \
                                  f'{utils.bytes_to_readable_unit(pathlib.Path(segment_partial_name).stat().st_size)}.  Aborting download.'
                        logging.error(message)
                        raise internet.IncompleteDownload(message)
                    else:
                        self.complete = True
                        logging.info(f'Completed downloading of segment: {segment_partial_name}. '
                                     f'Size: {utils.bytes_to_readable_unit(pathlib.Path(segment_partial_name).stat().st_size)}')
...
class DownloadJob(threading.Thread):
    ...
    def __merge_completed(self):
        ...
        logging.info(f'Completed download of {self.config.full_destination_name} from {self.config.source_url}. '
                     f'Total size: {utils.bytes_to_readable_unit(pathlib.Path(self.config.full_destination_name).stat().st_size)}')

    def run(self) -> None:
        logging.info(f'Staring download of {self.config.source_url}. Size is: {utils.bytes_to_readable_unit(self.config.file_length)}')
        self.__lock.acquire()
        for n in range(self.config.worker_count):
