import cloudscraper
import logging
import digital_thought.commons.internet as internet
...
class Scrapper(sessions.Session):

    def __init__(self, tor_proxy=None, internet_proxy=None) -> None:
        super().__init__()
        if tor_proxy is None:
            tor_proxy = {"http": 'socks5h://127.0.0.1:9150'}
        self.tor_scrapper = cloudscraper.create_scraper(sess=internet.retry_request_session(proxy=tor_proxy))
        self.internet_scrapper = cloudscraper.create_scraper(sess=internet.retry_request_session(proxy=internet_proxy))
    ...
    def requester(self, url) -> cloudscraper.Session:
        try:
